...
if __name__ == "__main__":
    ...
    nano = YoloNano(class_num=2)
    if os.path.exists('yolonano1.pkl'):
        nano.load_state_dict(torch.load('yolonano1.pkl', map_location=torch.device('cpu')))
    ...
    for i in img_list:
        ...
        output1,output2,output3,loss = nano(imginput)
        output1 = output1[0]
        output2 = output2[0]
        output3 = output3[0]
        ...
        if result:
            ...
            #     rest = nms(item, nms_threshold)
            #     result += rest
            # print('use nms result :', len(result))

            for r in result:#第三类及以上为黑色框
